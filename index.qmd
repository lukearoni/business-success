...
ggplot(plot_data, aes(x = reorder(NAME, pop_growth_rate), y = pop_growth_rate)) +
  ...
  scale_fill_manual(values = c("TRUE" = "#33a02c", "FALSE" = "#2c7fb8"), guide = "none") +
...
ggplot(combined_data, aes(x = reorder(Name, PERINCREASE), y = PERINCREASE)) +
  ...
  geom_text(aes(label = paste0(round(PERINCREASE, 1), "%")), 
            hjust = -0.1, size = 3) +
  labs(
    title = "Top 10 Regions by Building Permit Percent Increase (2020 - 2022)", 
    subtitle = "Sheridan, WY and the median are included for comparison. Sheridan is much higher than the median, but not in the top 10",
    x = NULL,
