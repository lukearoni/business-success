...
```{r}
# Step 1: Select top 10 by PERINCREASE
top10 <- final_table %>%
  filter(!is.na(PERINCREASE)) %>%
  slice_max(order_by = PERINCREASE, n = 10)

# Step 2: Plot bar graph
ggplot(top10, aes(x = reorder(Name, PERINCREASE), y = PERINCREASE)) +
  geom_col(fill = "#1f78b4") +
  coord_flip() +
  geom_text(aes(label = paste0(PERINCREASE, "%")), hjust = -0.1, size = 3) +
  labs(
    title = "Top 10 Regions by Percent Increase",
    x = NULL,
    y = "Percent Increase"
  ) +
...
# Get Sheridan WY row exactly
sheridan_row <- final_table %>%
  filter(tolower(Name) == "Sheridan, WY") %>%
...
# Combine rows WITHOUT removing duplicates
combined_data <- bind_rows(
  top10,
  tibble(Name = "Median", PERINCREASE = median_value),
  sheridan_row
)

# To ensure Sheridan shows, give Sheridan a unique label if it’s duplicated in top10
combined_data <- combined_data %>%
  mutate(Name = ifelse(duplicated(Name) & Name == "Sheridan, WY", "Sheridan, WY (extra)", Name))

# Plot
ggplot(combined_data, aes(x = reorder(Name, PERINCREASE), y = PERINCREASE)) +
  geom_col(aes(fill = case_when(
    Name == "Median" ~ "#e31a1c",
    grepl("Sheridan, WY", Name) ~ "#33a02c",
    TRUE ~ "#1f78b4"
  ))) +
  coord_flip() +
  geom_text(aes(label = paste0(round(PERINCREASE, 1), "%")), hjust = -0.1, size = 3) +
  labs(
